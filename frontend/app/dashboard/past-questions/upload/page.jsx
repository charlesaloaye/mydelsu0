...
function UploadPastQuestionsPage() {
  ...
  // Load notifications and submissions
  useEffect(() => {
    if (isAuthenticated) {
      loadNotifications();
      loadSubmissions();
  ...
  const loadSubmissions = async () => {
    try {
      ...
      const response = await apiClient.getMyPastQuestions();
        ...
        const submissions = (response.data || []).map((submission) => ({
          ...
          earnings:
            submission.status === "approved"
              ? parseFloat(submission.price || 0)
              : 0,
          date: submission.created_at,
          reason: submission.admin_notes || null,
          file_name: submission.file_name,
        }));
        console.log("Processed submissions:", submissions); // Debug log
        setPreviousSubmissions(submissions);
      } else {
        console.error("Failed to load submissions:", response.message);
        setPreviousSubmissions([]);
      }
    } catch (error) {
      console.error("Error loading submissions:", error);
      console.error("Error details:", error.message, error.response); // Debug log
      setPreviousSubmissions([]);
    } finally {
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>• Upload a single PDF, DOC, or DOCX file</li>
