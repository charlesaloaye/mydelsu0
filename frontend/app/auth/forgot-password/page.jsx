...
function ForgotPasswordPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiClient.forgotPassword(formData.emailOrPhone.trim());
      setStep("sent");
    } catch (e) {
      setErrors((prev) => ({
        ...prev,
        emailOrPhone:
          e?.response?.message || e?.message || "Failed to send reset link",
      }));
    }
  };
                ...
                {formData.resetMethod === "whatsapp" ? (
                  (() => {
                    const encoded = encodeURIComponent(
                      `Hi, I forgot my password and need help resetting it. My login email/phone is: ${formData.emailOrPhone}`
                    );
